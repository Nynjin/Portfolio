    ...
    <div className="fixed inset-0 pointer-events-none z-50 mix-blend-hard-light">
      {particles.map((particle) => (
        <motion.div
          ...
          animate={{
            ...
            opacity: 0,
            x: [
              `${particle.x.toString()}vw`,
              `${(particle.x + Math.random() * 30 - 15).toString()}vw`,
              `${particle.x.toString()}vw`,
            ],
          }}
          transition={{
      ...
      <motion.div
        style={{
          ...
          height: '100%',
          background:
            'linear-gradient(to bottom, rgba(255, 255, 255, 0) 0%, rgba(37, 99, 235, 0.5) 250vh)',
        }}
        initial={{ opacity: 0 }}
